							...
							<c:choose >
								...
								<c:otherwise>
									<c:if test="${loginMember.email eq 'admin' }">
										<a href="ShowMember.jsp">전체회원조회</a>
									</c:if>
								...
								<div class="inner">
									...
									<%
										...
										if(loginMember != null){
											...
											messageList = new MessageDAO().messageList(loginMember.getEmail());
									...
									<c:forEach items="${messageList }" var="message" varStatus="s">
										<tr>
											...
											<td><a href="DeleteMessage.do?num=${message.num }">삭제</a></td>
										</tr>
									</c:forEach>
									</table>
									...
									<%
										if(loginMember != null){%>
											...
											<%for(int i=0; i<messageList.size(); i++){ %>
												<tr>
													<td><%=i+1 %></td>
													<td><%=messageList.get(i).getSender() %></td>
													<td><%=messageList.get(i).getMessage() %></td>
													<td><%=messageList.get(i).getM_date() %></td>
												</tr>
											<%} %>
											</table>
										<%} %>
							...
							<!-- 다른 사람의 DB에 메시지 저장해보기! -->
								...
								<c:choose>
									<c:when test="${!empty loginMember }">
										...
										<input type="text"  id="name" value="${loginMember.email }" name="sender" />
									</c:when>
									<c:otherwise>
										<label for="name">Name</label>
									...
									<div class="field half">
										...
										<input type="text"  id="email" placeholder="받는 사람 이메일" name="receiver"/>
									</div>
		...
		<!-- Scripts -->
			...
			<script type="text/javascript">
				function checkE(){
					...
					// {key1 : vlaue1, key2 : value, {key3 : value3}}
				$.ajax(
					{
						// 어디로 요청할 건지
						url : "EmailCheck.do",
						...
						success : function(data){
							if(data=='true'){
								$('#checkE').html('사용가능한 이메일입니다~')
							}else{
								$('#checkE').html('사용불가능한 이메일입니다~')
							}
						},
						error : function(){
							alert("통신실패!")
