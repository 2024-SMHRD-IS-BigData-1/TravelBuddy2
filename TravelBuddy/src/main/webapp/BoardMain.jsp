...
<body>	
			...
			<%List<Board> boardList =  new BoardDAO().showBoard();
			...
			<div id="board">
				...
				<c:forEach items="${boardList }" var="board" varStatus="s">
                  <tr>
                     ...
                     <td>${board.writer }</td>
                     <td>${fn:split(board.b_date," ")[1] }</td>
                     
                     <c:if test="${loginMember.email eq 'admin' }">
                     	<td><a href="BoardDelete.do?num=${board.num }">삭제</a></tr>
                     </c:if>
               </c:forEach>
				...
				<% for(int i=0; i<boardList.size(); i++){ %>
					<tr>
						...
						<td><%=boardList.get(i).getB_date().split(" ")[0] %></td>
					</tr>
				<%} %>
				</table>
